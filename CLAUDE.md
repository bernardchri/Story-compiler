...
- Mockups/images: use standard markdown `![alt](filename.png)` with a path **relative to the STORIES.md file**. During compilation, paths are automatically rewritten to be relative to the project root so they resolve correctly in both `README.md` and `README.pdf`. Recommended convention: place images alongside the `STORIES.md` (e.g. `src/my-module/mockup-home.png`).
